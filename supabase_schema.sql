...
CREATE POLICY "Users can CRUD own properties" ON public.properties FOR ALL USING (auth.uid() = user_id OR public.is_admin());

-- Update Policies for Bookings (Admins can view all)
DROP POLICY IF EXISTS "Users can CRUD own bookings" ON public.bookings;
CREATE POLICY "Users can CRUD own bookings" ON public.bookings FOR ALL USING (auth.uid() = user_id OR public.is_admin());
